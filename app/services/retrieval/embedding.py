...
def embed_query(query: str) -> list[float]:
    _init()
    dim = _GEMINI_DIM if _model_type == "gemini" else _FALLBACK_DIM
    with logfire.span("🔢 Query Embedding") as span:
        if _model_type == "gemini":
            result = _active_model.embed_query(query)
        else:
            result = _active_model.encode([query])[0].tolist()
        if span:
            span.set_attribute("embedding.model", _model_type)
            span.set_attribute("embedding.dimensions", dim)
    return result
